...
from ai_project_poker.engine.round_manager import RoundManager
from ai_project_poker.engine.game_evaluator import GameEvaluator
from ai_project_poker.engine.poker_constants import PokerConstants as Const
from ai_project_poker.engine.player import Player
from ai_project_poker.engine.pay_info import PayInfo
from ai_project_poker.engine.card import Card
from ai_project_poker.engine.deck import Deck
from ai_project_poker.engine.table import Table
...
class RoundManagerTest(BaseUnitTest):
  ...
  def test_message_after_start_round(self):
    with patch('ai_project_poker.engine.message_builder.MessageBuilder.build_round_start_message', return_value="hoge"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_street_start_message', return_value="fuga"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_ask_message', return_value="bar"):
  ...
  def test_message_after_apply_action(self):
    with patch('ai_project_poker.engine.message_builder.MessageBuilder.build_round_start_message', return_value="hoge"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_street_start_message', return_value="fuga"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_ask_message', return_value="bar"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_game_update_message', return_value="boo"):
      state, _ = self.__start_round()
  ...
  def test_message_after_forward_to_flop(self):
    with patch('ai_project_poker.engine.message_builder.MessageBuilder.build_street_start_message', return_value="fuga"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_ask_message', return_value="bar"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_game_update_message', return_value="boo"):
      state, _ = self.__start_round()
  ...
  def test_state_after_showdown(self):
    mock_return = [1,0]*3
    with patch('ai_project_poker.engine.hand_evaluator.HandEvaluator.eval_hand', side_effect=mock_return),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_round_result_message', return_value="bogo"):
      state, _ = self.__start_round()
      state, _ = RoundManager.apply_action(state, "fold", 0)
      state, _ = RoundManager.apply_action(state, "call", 10)
      state, _ = RoundManager.apply_action(state, "call", 10)
  ...
  def test_message_after_showdown(self):
    ...
    with patch('ai_project_poker.engine.hand_evaluator.HandEvaluator.eval_hand', side_effect=mock_return),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_game_update_message', return_value="boo"),\
         patch('ai_project_poker.engine.message_builder.MessageBuilder.build_round_result_message', return_value="foo"):
      state, _ = self.__start_round()
      state, _ = RoundManager.apply_action(state, "fold", 0)
      state, _ = RoundManager.apply_action(state, "call", 10)
      state, _ = RoundManager.apply_action(state, "call", 10)
